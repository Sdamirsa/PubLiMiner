...
class DeduplicateStep(StepBase):
    ...
    def run(self) -> StepMeta:
        ...
        df = pl.read_parquet(self.spine.parquet_path, memory_map=False)
        meta.rows_before = len(df)
        removed: list[dict[str, str]] = []

        # Layer 1: PMID exact duplicates
        with ProgressReporter("dedup_pmid", total=1, desc="Layer 1/4: PMID exact") as p:
            before = len(df)
            df = df.unique(subset=["pmid"], keep="first")
            pmid_dupes = before - len(df)
            p.advance()
        if pmid_dupes > 0:
            self.logger.info(f"Layer 1: removed {pmid_dupes} PMID duplicates")
            removed.extend(
                [{"pmid": "unknown", "reason": "pmid_duplicate"}] * pmid_dupes
            )

        # Layer 2: DOI exact duplicates
        if self.config.check_doi and "doi" in df.columns:
            with ProgressReporter("dedup_doi", total=1, desc="Layer 2/4: DOI exact") as p:
                to_remove = _find_doi_duplicates(df)
                p.advance()
            if to_remove:
                df = df.filter(~pl.col("pmid").is_in(to_remove))
                self.logger.info(f"Layer 2: removed {len(to_remove)} DOI duplicates")
                removed.extend(
                    [{"pmid": pmid, "reason": "doi_duplicate"} for pmid in to_remove]
                )

        # Layer 3: Title fuzzy match (with exact-collapse + blocking optimization)
        if self.config.check_title_fuzzy and "title" in df.columns:
            # 3a: cheap exact-title collapse first (Polars vectorized)
            before = len(df)
            df = df.with_columns(
                pl.col("title").str.to_lowercase().str.strip_chars().alias("_title_norm")
            )
            df = df.unique(subset=["_title_norm"], keep="first").drop("_title_norm")
            exact_title_dupes = before - len(df)
            if exact_title_dupes > 0:
                self.logger.info(
                    f"Layer 3a: removed {exact_title_dupes} exact-title duplicates"
                )
                removed.extend(
                    [{"pmid": "unknown", "reason": "exact_title_duplicate"}]
                    * exact_title_dupes
                )

            # 3b: blocked fuzzy match for near-duplicates
            to_remove = _find_fuzzy_title_duplicates(
                df, threshold=self.config.fuzzy_threshold, logger=self.logger,
            )
            if to_remove:
                df = df.filter(~pl.col("pmid").is_in(to_remove))
                self.logger.info(f"Layer 3b: removed {len(to_remove)} fuzzy title duplicates")
                removed.extend(
                    [{"pmid": pmid, "reason": "fuzzy_title_duplicate"} for pmid in to_remove]
                )

        # Layer 4: Retracted papers
        if self.config.remove_retracted and "publication_status" in df.columns:
            ...
            if len(retracted) > 0:
                ...
                df = df.filter(~retracted_mask)
                self.logger.info(f"Layer 4: removed {len(retracted_pmids)} retracted papers")
                removed.extend(
        ...
        total_removed = meta.rows_before - len(df)
        ...
        meta.extra["fuzzy_duplicates"] = len([r for r in removed if r["reason"] == "fuzzy_title_duplicate"])
        meta.extra["retracted"] = len([r for r in removed if r["reason"] == "retracted"])

        self.logger.info(f"Deduplication complete: {meta.rows_before} -> {len(df)} rows ({total_removed} removed)")
        return meta
